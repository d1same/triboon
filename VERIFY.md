...
- Version contract: `package.json` 3.2.6; Android `versionName` 3.2.6 /
  `versionCode` 383; Windows client package/Tauri/Cargo(.lock) 3.2.6.
- The handshake says Jellyfin 10.11.11, which those apps can read. Behind a
  proxy it keeps the https address the app typed.
- Gate: `npm.cmd test` 726/726. `npm.cmd run verify:full` PASS —
  whitespace, JS syntax, web parse, IPTV/P9, VOD/P14, CC/P11, full suite,
  isolated `/api/server` 3.2.6, household VOD play/seek/resume/CC, household
  IPTV first-byte + retune, household overlapping Play, Android lint +
  unit tests + debug build, Android ExoPlayer stress on `emulator-5554`
  (never the Shield). Windows GPU/HDR was not run.
...
- Version contract: `package.json` 3.2.4; Android `versionName` 3.2.4 /
  `versionCode` 381; Windows client package/Tauri/Cargo(.lock) 3.2.4.
- Gate: `npm.cmd test` 721/721. `npm.cmd run verify:full` PASS —
  whitespace, JS syntax, web parse, IPTV/P9, VOD/P14, CC/P11, full suite,
  isolated `/api/server` 3.2.4, household VOD play/seek/resume/CC, household
  IPTV first-byte + retune, household overlapping Play, Android lint +
  unit tests + debug build, Android ExoPlayer stress on `emulator-5554`
  (never the Shield). Household live used the Unraid server. Windows
  GPU/HDR was not run.
...
Play joins the already-warmed mount:

- Version contract: `package.json` 3.2.2; Android `versionName` 3.2.2 /
  `versionCode` 379; Windows client package/Tauri/Cargo(.lock) 3.2.2.
- Gate: `npm.cmd test` 721/721. `npm.cmd run verify:full` against
  `http://127.0.0.1:7799` (repo 3.2.2) + `emulator-5554` (never the Shield):
  every gate PASS — whitespace, JS syntax, web parse, IPTV/P9, VOD/P14,
  CC/P11, full suite, isolated `/api/server` 3.2.2, household VOD
  play/seek/resume/CC (Mario 4K + FROM remux; resume 32ms/12ms), household
  IPTV first-byte + retune, household overlapping Play, Android lint +
  unit tests + debug build, Android ExoPlayer stress. House :7777 left on
  3.1.26.
...
- Version contract: `package.json` 3.1.22; Android `versionName` 3.1.22 /
  `versionCode` 367; Windows client package/Tauri/Cargo 3.1.22.
- Search keeps the typed query and spell-checks words. Example: `office`
  shows The Office, `oddyse` shows Odyssey, `longst yard` stays The
  Longest Yard instead of Lord of the Rings.
- D-pad walks field → chips → posters. Up from the first poster lands on
  the last chip. Right on a chip stays put (no jump to the X). Back from
  posters lands on the box.
- `npm.cmd run verify:full` against this repo on `http://127.0.0.1:7799`
  and emulator `emulator-5554` (not the Shield). Node suite **685/685**.
  Isolated `/api/server` 3.1.22.
- Household VOD Mario 4K + FROM S01E01 play/seek/resume/CC PASS (Mario
  ready 18ms; FROM ready 1813ms; resume 15ms/10ms). IPTV web+native
  retune PASS (8181 channels). Overlapping Play PASS (both ready in 21ms).
  Android lint/unit/debug build PASS. Android ExoPlayer stress on
  `emulator-5554` PASS. Windows GPU not instrumented.
