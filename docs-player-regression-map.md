...
- Android normal Live TV remains native ExoPlayer-only, but Multiview is a
  ...
  before mounting panes. A slow provider/codec `ExoPlayer.release()` must run
  after this visible-surface acknowledgement so it cannot hold an opaque native
  layer over an otherwise-ready Multiview page. If MediaSource is unavailable,
  Multiview fails closed before mounting any pane.
...
| P14 | Multi-user VOD performance is capacity-managed instead of connection-count-only. Provider limits are saved per account up to the current server cap, multiple usenet providers combine without losing their individual caps, and Settings -> Streaming performance owns expected users, remote users, quality mix, bandwidth, buffer targets, per-stream connection windows, and start/seek reserve. The recommendation flow must use the server-side provider list and return plain owner-facing guidance. NNTP scheduling must prioritize startup/seek work before playback, playback before health, and health before read-ahead. Two or three concurrent Plays share indexer fan-out and a small startup-mount gate so one title's dead-source race cannot starve another Play's first frame. Playback read-ahead may grow when the server is idle, but it must shrink under active-user pressure so one large 4K stream cannot starve another user's first frame or seek. Decoded VFS cache retention is byte-capped as well as segment-capped because usenet article segment sizes vary by release; do not tune read-ahead without the byte budget. Detail-page source warmup must remain reusable by exact-id and TV-episode Play requests, including string-vs-number season/episode route values. `/api/prepare` may prepare only the stable detail Play target, may walk only a small capped ranked-source slice to skip a bad top pick, must expose no stream URL or play session, and Play must reuse or join the prepared/in-flight mount plus in-flight NZB prefetch instead of duplicating search, probe, mount, or health-gate work. TV playback must prepare the exact next episode once inside the final 120 seconds, warm/join its local-library lookup separately, and let manual/autoplay Play join that prepared or in-flight work without a second fan-out/NZB fetch/mount. Health checks keep the bounded gate and original background triage, but health/read-ahead work must not outrank the active segment being streamed. `docs-streaming-performance.md` is the canonical tuning/reference document; do not replace this with old fixed 16-connection or 8-12 read-ahead assumptions from historical benchmarks. | `server/index.js` `MAX_PROVIDER_CONNECTIONS`, `normalizeProviders`, `normalizeStreamingPerformance`, `recommendStreamingPerformance`, `streamingRuntimeProfile`, `/api/streaming/recommend`, `/api/prepare`, `/api/play`, provider settings; `server/pipeline.js` `Pipeline.search`, `Pipeline.prepare`, `prepareInflight`, adaptive read-ahead, source/NZB warmup reuse, and health probe count; `server/nntp.js` provider priority lanes and least-loaded provider ordering; `server/vfs.js` playback vs read-ahead priorities and decoded-cache byte cap; `server/archive.js` health priority; `web/index.html` `prefetchSources`, `preparePlaybackSource`, `prepNextEpisode`, `maybePrepareNextEpisode`, `playbackRequestBody`, Streaming performance settings card; `docs-streaming-performance.md` | `test/security.test.js` streaming performance recommendation, route coverage, and high-connection provider round-trip; `test/e2e.test.js` NNTP priority queue/cache byte cap; `test/phase2.test.js` pipeline/source warmup, fallback prepare reuse, and TV episode cache-key regression; `test/phase4.test.js` detail prepare/request-builder and near-end episode prepare contracts; full `npm.cmd test`; multi-user playback stress |
| P16 | In-player About is a player card, not a trip back to details. Movies open it from the About chrome button or Down from Play. Shows open it from the episode-strip About tile or the chrome button beside Guide. Hide About when TMDB has no people. Web, Android native, and the Windows catalog WebView share `openPlayerAbout` / `closePlayerAbout`; Android native paints its own overlay through `showTitleInfo`. Opening and closing About must not stop the stream. | `web/index.html` `openPlayerAbout`, `closePlayerAbout`, `playerAboutCastList`, `aboutBtn`; `MainActivity.java` `showTitleInfo`, `showNativeTitleInfo`; `bench/android-tv-stress.ps1` About open/close | `test/phase4.test.js`, `test/player-update-gate.test.js`, `test/verify-live.test.js`, `test/windows-px8-player.test.js`, Android stress About check |
| P15 | Windows desktop playback is a real native client, not a WebView preview. A narrow `TriboonTV` bridge is injected only for the validated configured Triboon origin; the remote page never receives global Tauri APIs, and commands accept only bounded payloads plus same-server media/subtitle URLs. One persistent libmpv owner and dedicated player surface replace items without showing details between episodes. The player mirrors tokened ready/playing/paused/progress/stats/seek/ended/closed/error/next/quality/episode/subtitle callbacks so the web state machine remains the single owner of Continue Watching, Trakt, Up Next, source recovery, and final checkpoints. D3D11 plus safe automatic hwdec must fall back to software; GPU decode may be claimed only when runtime `hwdec-current` reports a hardware decoder while frames advance. VOD covers direct/remux/transcode, absolute remount seeks, pause/resume, stalls, quality/audio/subtitles, and immediate next episode. Native Live TV tries provider HLS/TS candidates then ordered server fallbacks and releases old streams on retune. Mouse, keyboard, media keys, fullscreen, Back, and D-pad/controller controls remain reachable. The installer dynamically bundles the locked LGPL `libmpv-2.dll`, notices, verbatim license, and source/rebuild/replacement instructions. | `clients/windows-px8/src-tauri/src/main.rs`, `player.rs`, `ui/bridge.js`, native player UI; `web/index.html` `tryNativeVideoPlayer`, `tryNativeLivePlayer`, `__tvNativeVideo*`, `__tvNativeEpisodeSelect`; `.github/workflows/docker.yml`; `clients/windows-px8/LIBMPV-SOURCE.md` | `test/windows-px8-player.test.js`, `test/release-contract.test.js`, Rust unit tests with `--features player`, Windows 1080p/4K/HLS/TS hardware smoke and episode/CW/subtitle/input matrix |
...
| P13 | Android TV Back must preserve app context before leaving Triboon. The native wrapper routes `KEYCODE_BACK`, Activity `onBackPressed`, and Android 13+ `OnBackInvoked` through the same system-back handler so platform Back cannot bypass the web `__tvBack()` contract. Every visited title and cast/person page is an indexed same-document history entry: Details -> related title -> cast -> another work must unwind exactly one visited page per visible, keyboard, browser, or hardware Back, never jump to the original title, and never require a first Back merely to move to the top of a person grid. Direct deep links with no earlier in-app entry retain the safe detail/person origin fallback. Settings/Preferences and other non-home pages return Home inside Triboon; Movies, TV Shows, and attached-library pages first open the section/menu rail from the current scroll/focus position; only root Home answers exit with the press-back-again toast. Dialogs, player sheets, drawers, and track menus still close before page history. | `android/app/src/main/java/app/triboon/tv/MainActivity.java` `dispatchKeyEvent`, `onBackPressed`, `handleSystemBack`, `handleBack`; `web/index.html` `setRoute`, `backOneRoute`, `handleRouteTraversal`, `closeDetail`, `closePerson`, `__tvBack`, `backToBrowseSectionMenu`, `enterRail`, `switchView` | `test/phase4.test.js`, nested title/person Back sequence smoke, Android TV deep-person Back smoke, Android TV Settings Back smoke, Android TV Back smoke from Movies/TV/library mid-grid |
...
- **P14 - resume source pinning + playback-failed demotion.** The source that actually
  ...
  A human Sources tap still leads (width 1 so a 41GB remux is not raced against a WEB-DL).
  If that tap is dead — ISO / 7z / missing / encrypted — Play walks the remaining
  playable files in the drawer sort (Largest → next biggest). Named disc images are
  skipped before an NZB grab. The 4K-exhausted relax then applies after those 4K tries.
...
- **P11/P14 - native audio language.** The native player honors audio language as:
  ...
  ExoPlayer's real track list and in-place switching. Windows libmpv direct play
  sets `alang` from the same preference (`eng,en` by default) so ITA.ENG files
  start on English; the Audio menu switches with `set aid`. Native remux also
  applies the preference to `&audio=` before handoff, and a late track probe
  quietly respawns onto English if the stream started on track 0. Code:
  `web/index.html` `nativeAudioChoices`, `refreshNativeAudioChoices`,
  `maybeApplyNativePreferredAudio`, `__tvNativeVideoAudio`, native payload;
  `MainActivity.java` `applyNativeTrackSelectionDefaults`,
...
- **P14 - live connection grow and give-away.** Auto sizes each Play from
  `ceil(bitrate ÷ live Mbps/connection)` (floor 4, cap 24), not a fixed 10.
  Fast sockets stay few so they do not fill the home pipe. Slow evening
  sockets may grow a starving playhead from leftover spare, or take extras
  from a fat mount, never below 4 — but a live house fill at ≥75% of the
  safe download budget freezes grow. A healthy Play does not eat spare
  sockets. Coverage is bytes ahead of the last player read, not total cache
  (tail warmup and a just-started Play must not look fat or dump a healthy
  4K). Steal waits for an 8s hold plus 5s cooldown; a new viewer first gets
  a need-weighted share. Custom mode keeps even split using the typed caps.
  A recent 502 also freezes grow. A fully cached small file drips to the
  floor while still playing. Startup/seek also pick the provider that can
  fit that Play (4K ~18 leftover slots, 1080 ~10) so Easynews at 40/50 does
  not 502 a 4K that Newshosting can take. Code:
  `server/pipeline.js` `allocateStreamConnections`, `classifyStreamNeed`,
  `fileIsFullyAhead`; `server/nntp.js` `TransferMeter`; `server/vfs.js`
...
- **P14 - playback resource isolation.** Viewer fairness is driven only by real
  ...
  parent-mount lifecycle. A local-library Play is a live player for eviction, but
  it is not a usenet viewer: it must not take NNTP sockets or shrink another
  stream's cache. ffmpeg remux/transcode of an added file reads the disk path. Prepared mounts keep a bounded 4-read-ahead,
