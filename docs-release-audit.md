...
- High-volume Movies, TV Shows, and Watchlist poster grids must stay windowed: the client may keep the full logical item list for focus/playback, but only the visible rows plus overscan should be mounted in the DOM.
- The left rail must keep Add library, Preferences, Settings, and the active profile in a pinned footer; library/menu overflow belongs only in the scrollable rail body so 720p TV users always have a visible way to switch profile or open settings.
...
- Streaming/session follow-up: HTTPS IPTV live-remux no longer hands ffmpeg `https://<pinned-ip>/...`, preserving provider TLS SNI while keeping pinned-IP remux for plain HTTP and pinned lookup for Node fetch/proxy paths. `/api/stream` now classifies initial reads as `startup` and non-zero ranges as `seek`, boosts the first needed NNTP article fetch through that lane, then returns to normal playback priority while cancelling future read-ahead scheduling when the player seeks away or disconnects. Browser sessions now mirror the token into `sessionStorage` as a fallback and only clear saved login on a real 401, so a slow/transient `/api/me` boot failure cannot force a fresh login after refresh. Verification passed `node --test test/e2e.test.js` 15/15, `node --test test/phase4.test.js` 19/19, `node --test test/security.test.js` 70/70, `node --check server/index.js`, `node --check server/vfs.js`, and inline `web/index.html` script parse. A disposable local `/api/server` timing probe returned 89ms then 8-10ms cached responses; the in-app browser policy blocked direct visual smoke on the disposable localhost port, so no browser-login visual pass is claimed for this item.
- Release v1.7.1 verification: local Node LTS was updated to `v24.17.0` and the full suite passed under that runtime (`npm.cmd test` 201/201). Android `assembleDebug` built a universal APK with `versionCode='65'` and `versionName='1.7.1'`; `apksigner verify --print-certs` passed with the debug signing certificate used by the current public APK channel. Release assets `triboon-tv-v1.7.1.apk`, `triboon-tv.apk`, `triboon-mobile-v1.7.1.apk`, and `triboon-mobile.apk` share SHA-256 `0409192190FFC405C2A600B95208AE9ADC774AAD4E42B77C00FF022C33085223`. This release covers safe IPTV web-remux redirect resolution, centered live-stream error UI, Android native playback stats/update links, local-library SQLite lookup coverage, and the activity `sessionId` API alias used by the admin now-watching panel.
- Release v1.7.3 verification: Android 4K/native VOD crash audit reproduced a real emulator fatal caused by nested ExoPlayer error callbacks releasing a null player after a codec/buffer failure. Native playback release is now one-shot and null-safe, sustained post-start rebuffer trims Android caches after 15s and retries the same source/mode at the same timestamp after 45s, and heavy VOD uses shorter back buffers plus a bounded memory target instead of retaining oversized decoded segments. Server mounts now cap decoded VFS cache by bytes as well as segment count and adapt the byte budget to active mounts, so large 4K posts cannot grow memory by decoded article count alone. Emulator stress after the fix kept native playback alive for 90s and survived repeated skip forward/back without FATAL/ANR/NPE logs; the emulator still rejected some 4K HEVC formats by hardware capability, which now falls into the handled retry path instead of crashing the app. Verification passed full Node suite `node --test ...` 206/206, `git diff --check`, Android `assembleDebug`, `aapt dump badging dist/triboon-tv-v1.7.3.apk` reporting `versionCode='67'` and `versionName='1.7.3'`, and `apksigner verify --print-certs` with debug certificate SHA-256 `51d2f7b7075dcca63a2b99dc5b2c4918283f5b03eef8eaa1e91338dd78113d8f`. Release assets `triboon-tv-v1.7.3.apk`, `triboon-tv.apk`, `triboon-mobile-v1.7.3.apk`, and `triboon-mobile.apk` share SHA-256 `52CD05A29AFE5141BAE80D3343A0DB762AFF164B42449771FB51D579C4986F36`.
