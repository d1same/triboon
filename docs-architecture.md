...
| Streaming engine | `server/nzb.js`, `server/nntp.js`, `server/vfs.js`, `server/rar.js`, `server/zip.js`, `server/archive.js`, `server/par2.js`, `docs-streaming-performance.md` | Clean-room NZB mount, requested-episode selection inside season-pack archives, article reads, RAR/ZIP extent maps, obfuscated volume sets renamed/ordered from par2 FileDesc hashes or RAR5 volume numbers, Range streaming, provider capacity, priority lanes, lost-login (480) socket rebuild, a two-minute pause on new logins that keeps the provider plan, a per-account fresh-login breaker, adaptive read-ahead. |
...
| Music Home | `server/ytmusic.js`, `server/index.js`, `web/index.html` | YouTube Music search/home/charts via optional `ytmusicapi`; public search/radio without an account; lyrics + like (linked cookies); encrypted per-user imported browser-cookie sessions for personal playlists; bounded `yt-dlp` playback resolver, tokenized audio proxy, full-screen now-playing (compact queue), and no ExoPlayer handoff for audio yet. |
